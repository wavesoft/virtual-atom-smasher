...
define(
	...
	[ 
		...
		"text!vas/basic/tpl/screen/block/simulation/observable_point.html"
	],
	...
	function( MathJax, R, UI, DataWidget, Calculate, tpl ) {
		...
		/**
		 * The circular target in the simulation status screen
		 * 
		 * @class
		 * @classdesc The circular target in the simulation status screen
		 * @registry screen.block.observable_point
		 */
			...
			this._handleTimer = setTimeout((function() {
				...
				UI.showPopup( 
					...
					(function(hostDOM) {
						...
						// Prepare the body
						var comBody = R.instanceComponent("popup.observable", hostDOM);
						if (comBody) {
		...
		R.registerComponent( "screen.block.observable_point", ObservableWidget, 1 );
