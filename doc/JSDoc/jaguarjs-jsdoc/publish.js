 ...
 */
function buildNav(members, registryMembers) {
    var nav = [];

    if (registryMembers.length) {
        _.each(registryMembers, function (v) {
            nav.push({
                type: 'registry',
                longname: v.longname,
                name: v.registry,
    ...
    if (members.classes.length) {
        _.each(members.classes, function (v) {
            if (registryMembers.indexOf(v) >= 0) return;
            nav.push({
                type: 'class',
                longname: v.longname,
                name: v.name,
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    // Post-process members and find registry items
    var registryMembers = [];
    data().each(function(doclet) {
        if (doclet.registry) {
            registryMembers.push(doclet);
        }
    });
